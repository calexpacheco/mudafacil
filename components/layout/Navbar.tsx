    ...
    <header className="sticky top-0 z-50 border-b border-gray-200 bg-white/90 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-gray-600">
          <Link href="/#features" className="hover:text-gray-900 transition-colors font-medium">{t('features')}</Link>
          <Link href="/#pricing"  className="hover:text-gray-900 transition-colors font-medium">{t('pricing')}</Link>
        </nav>
          ...
          {session ? (
            <>
              ...
              <form action={async () => { 'use server'; await signOut({ redirectTo: '/' }) }}>
                <button
                  type="submit"
                  className="text-sm px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors font-medium cursor-pointer"
                >
                  {t('signOut')}
                </button>
              </form>
